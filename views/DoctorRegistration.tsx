...
export const DoctorRegistration: React.FC<DoctorRegistrationProps> = ({ onBack, onLoginClick }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!formData.bmdcNumber || !formData.specialization) {
        setError('BMDC Number and Specialization are required');
        setIsLoading(false);
        return;
      }

      // Call Doctor Registration API (API-First Architecture)
      const response = await api.registerDoctor({
        ...
        onlineFee: formData.onlineFee,
        physicalFee: formData.physicalFee,
        consultationFee: formData.physicalFee || formData.onlineFee
      });
