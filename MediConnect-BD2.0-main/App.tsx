...
const App = () => {
  ...
  const handlePatientLoginSuccess = (userData: any) => {
     ...
     if (returnTo === 'booking') {
       // User was booking an appointment before login
       setCurrentView('patient');
       // The draft will be restored in PatientPortal component
     } else {
       setCurrentView('patient');
     }
  };
  ...
  const renderView = () => {
    ...
    if (!currentUser) {
      // Allow guest browsing for patient portal
      if (currentView === 'patient' || currentView === 'patient_appointments' || 
          currentView === 'patient_settings' || currentView === 'medical_history') {
        return <PatientPortal currentUser={null} onNavigate={setCurrentView} onBack={handleLogout} initialMode="DASHBOARD" />;
      }
