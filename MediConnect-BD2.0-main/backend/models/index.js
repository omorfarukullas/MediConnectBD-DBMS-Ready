...
// ===== NEW SCHEMA MODELS (Active) =====
const Patient = require('./Patient');
const DoctorNew = require('./DoctorNew');
const AppointmentNew = require('./AppointmentNew');

// ===== OLD MODELS (Commented out to prevent crashes) =====
...
// ===== NEW SCHEMA RELATIONSHIPS (Active) =====
// Patient <-> Appointments (One-to-Many)
Patient.hasMany(AppointmentNew, { foreignKey: 'patient_id', as: 'patientAppointments' });
AppointmentNew.belongsTo(Patient, { foreignKey: 'patient_id', as: 'patient' });

// Doctor <-> Appointments (One-to-Many)
DoctorNew.hasMany(AppointmentNew, { foreignKey: 'doctor_id', as: 'doctorAppointments' });
AppointmentNew.belongsTo(DoctorNew, { foreignKey: 'doctor_id', as: 'doctor' });

// ===== OLD RELATIONSHIPS (Commented out) =====
/*
...
module.exports = {
    sequelize,
    // ===== NEW SCHEMA EXPORTS (Active) =====
    Patient,
    DoctorNew,
    AppointmentNew,
    // ===== OLD EXPORTS (Available but not actively used) =====
    User,
    Doctor,
    Hospital,
    Appointment,
